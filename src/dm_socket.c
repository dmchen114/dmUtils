...
#ifdef WIN32
		if(g_hWndNotify)
			PostMessage(g_hWndNotify, WM_SOCKET_CLOSE_THREAD, 0, 0);
#endif
		//Should wait for the thread to exit!
		JoinNetThread();

#ifdef WIN32
		lstDeleteAll(g_sockMap);
		g_sockMap = NULL;
...
int FillFdSets(fd_set* fsRead, fd_set* fsWrite, fd_set* fsException)
{
	...
	if(g_sockMap == NULL)
		return -1;
	lstLock(g_sockMap);
	it = lstFirst(g_sockMap);
	while(it != NULL)
	{
		LPUDPTRANSPORT trans = (LPUDPTRANSPORT)lstRetrieve(it);
...
void RegistHandler(LPUDPTRANSPORT trans)
{
	...
	{
		lstLock(g_sockMap);
		lstInsert(g_sockMap, trans, lstHeader(g_sockMap));
		lstUnLock(g_sockMap);
	}
#ifndef USE_SELECT
	WSAAsyncSelect(trans->s, g_hWndNotify, WM_SOCKET_P2PUDP_SOCKET, FD_READ | FD_WRITE | FD_ACCEPT | FD_CLOSE);
#else
	...
	if(trans->transType == eTransportTCP || trans->transType == eTransportConnector)
		trans->epoll_event.events |= (EPOLLET | EPOLLOUT);
...
void OnTCPInput(LPUDPTRANSPORT trans)
{
	...
	while(--nCount)
	{
		...
		if(trans->pSink.pPtr != NULL){
			int nClosed = trans->pSink.OnReceive(trans->pSink.pPtr, trans, (unsigned char*)s_buffer, nRet, trans->dwPeerIP, trans->wPeerPort);
			if(nClosed)
				break;
		}
		if (nRet < 64*1024)
...
int SendUdpData(LPUDPTRANSPORT trans, char* buf, int len, unsigned long dwPeerIP, unsigned short port)
{
	...
	if(nRetVal == SOCKET_ERROR)
	{
		...
		case MY_EHOSTDOWN:
		case MY_EHOSTUNREACH:
		case MY_EAFNOSUPPORT:
			break;
		default:
			break;
		}
...
void DeleteTransportTimer(void* pPtr, unsigned long uID)
{
	LPUDPTRANSPORT trans = (LPUDPTRANSPORT)pPtr;
	free(trans);
}
...
void CloseUdpTransport(LPUDPTRANSPORT trans)
{
	if(trans != NULL && INVALID_SOCKET != trans->s)
	{
		...
		RegisterTimer(30, DeleteTransportTimer, trans, 1);
	}
}
